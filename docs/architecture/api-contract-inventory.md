...
| **F20** — no pagination on `GET /api/users` | **CLOSED by INF-262** (INF-250 decision). Opt-in pagination: `page`/`limit` → `findAndCountAll` + canonical envelope `{ data, pagination: { page, limit, total, totalPages } }`; without them the legacy full-array response is preserved (migration phase A) |
...
#### F49 — the sort parameters reach `ORDER BY` with no allowlist — **CLOSED by INF-262**

As recorded: `getAllUsers` passed `sortBy`/`sortOrder` straight into `order`, `?sortOrder[]=x` crashed to a 500 via `[].toUpperCase`, and nothing validated either parameter.

**Closed 2026-07-27 (INF-262):** `users.routes.js` now mounts `validateListUsers` on `GET /` — off-whitelist `sortBy`, malformed `sortOrder` (including array-shaped values), and every other invalid query parameter return a deterministic 400 `E_VALIDATION`. As defense in depth the controller additionally maps `sortBy` through `USER_LIST_SORT_COLUMNS` (fallback `created_at`) and coerces `sortOrder` to `ASC`/`DESC` (fallback `DESC`), so nothing client-supplied reaches the `ORDER BY` clause even if a validator is bypassed. Pinned by `tests/usersListSortContract.test.js` (rewritten from the defect pins, as that file itself required) and `tests/usersListQueryValidationContract.test.js`.
...
| GET | `/:id` | Admin, Management | 400, 401, 403, 404 | covered | covered | covered |
...
| **F13** | **The schema cannot be built from the repository.** Migrations create only **2 of the 17 tables** the models define — `auth_sessions` and `attendance_session_states`, both from 2026 migrations. The other 15, including `users`, `attendance`, `bookings` and `locations`, exist only in a baseline outside version control, and the oldest migration says so: *"Historical alignment stub: the users table already exists in the baseline schema."* | `src/models/migrations/*.cjs` |
...
| **F48** | **No migration guards against changes that already exist**, so a structure-only baseline dump cannot be migrated on top of. All nine call `addColumn`/`createTable` directly — zero use `describeTable` or `IF NOT EXISTS`. A `--no-data` dump copies the `sequelizemeta` *table* but not its *rows*, so the CLI re-runs every migration against a schema that already contains their changes. Verified empirically: empty ledger → *"Duplicate column name 'storage_provider'"*; ledger carrying 9 rows → migrations complete. **The baseline dump must carry the ledger rows** | `src/models/migrations/*.cjs`, `db/baseline/README.md` |
...
| **F39** | **The paginated list surfaces use different key names for the same concepts.** `GET /api/attendance` returns `total_records` and `records_per_page`; `GET /api/summary/reports` returns `total_items` and `items_per_page`. Since INF-262, `GET /api/users` uses the INF-250 **canonical** spelling — `page`, `limit`, `total`, `totalPages` — which the other two surfaces are expected to migrate to (separate scope). INF-267 explicitly preserves the attendance pagination keys for compatibility, so a client consuming all three must still handle three spellings | `attendanceRead.service.js` — `listManagementAttendances`; `summaryReport.service.js:405-412`; `user.controller.js` — `getAllUsers` |
...
| **F35** | **Users half CLOSED by INF-251/INF-261/INF-262.** OpenAPI now documents the slim projection and the full Phase A query/pagination matrix | `docs/openapi.yaml`; `user.controller.js` |
| **F36** | **CLOSED by INF-267.** `GET /api/attendance` now documents the validated query matrix, canonical audit-list projection, flat `data` array, sibling `pagination`, and separate detail operation. Pinned by `tests/attendanceManagementOpenApiContract.test.js` | `docs/openapi.yaml`; `attendance.routes.js`; `attendance.controller.js` |
...
| **F30** | **CLOSED by INF-267 route validation.** `validateManagementAttendanceList` rejects non-numeric, out-of-range, unknown, array, and object query values before `getAllAttendances` reaches the read service | `attendance.routes.js`; `src/modules/attendance/attendance.validation.js` |
...
| **F29** | **`target_date` is validated by shape, not by calendar — and the correct validator already exists.** The check is `/^\d{4}-\d{2}-\d{2}$/`, so `2026-13-45` and `2026-02-29` pass and reach a job that writes attendance state. `src/utils/isoDate.js` provides `parseIsoDateUtcStrict`, which rejects exactly these, and is already used by `historicalDateWindow.js`. The operational triggers hand-roll a weaker check instead | `attendance.controller.js:1866,1892,1918` vs `src/utils/isoDate.js` |
...
| **F20** | **`GET /api/users` pagination — CLOSED by INF-262** (INF-250 decision). Pagination is opt-in: `page`/`limit` switch the query to `findAndCountAll` (distinct count) with the canonical envelope `{ data: [...], pagination: { page, limit, total, totalPages } }`; without them the legacy full-array response is preserved as migration phase A. Filters (`role`, `program`, `division`, `position`, `location_status`) and email search landed in the same change. Pinned by `tests/usersListPaginationContract.test.js` and guarded in `tests/findingsRegisterGuard.test.js` | `src/controllers/user.controller.js` — `getAllUsers` |
...
| `GET /api/attendance` — query | `page`, `limit`, `search`, `from`, `to`, `mode`, `status`, `checkout_state`, `sortBy`, `sortOrder` | same |
| `GET /api/attendance` — body | flat `data` audit-list array plus sibling compatibility `pagination` | same |
| `GET /api/attendance/:id` — body | full audit detail, including detail-only email, notes, booking ID, and attendance-linked location | same |
...
**F36 (attendance half) — RESOLVED by INF-267.** OpenAPI now publishes the runtime query matrix, flat audit-list `data`, sibling pagination, and the separately authorized detail operation. `tests/attendanceManagementOpenApiContract.test.js` pins the public contract. The two list runtimes still spell pagination differently (users uses the INF-250 canonical `page/limit/total/totalPages`, attendance retains `total_records`/`records_per_page`), which is the explicit INF-267 compatibility decision recorded by F39 rather than an OpenAPI mismatch.
