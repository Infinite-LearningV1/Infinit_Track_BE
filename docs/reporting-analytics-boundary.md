...
| Excel/workbook-oriented export payload projection from the shared reporting source | `GET /api/summary/reports/excel` | Canonical Excel export contract. Shares the same backend report source logic as `/api/summary/reports`, but projects workbook metadata, summary sheet data, attendance rows, and discipline insight rows with runtime-calculated `recommended_action_code` values. |
